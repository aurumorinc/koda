...
class KodaClient:
    ...
    async def _scrape_internal(self, request: ScrapeRequest) -> ScrapeResponse:
        """Internal method to perform the scrape operation."""
    ...
    async def scrape(
        ...
        """
        effective_timeout = request.timeout or self.timeout
        request.timeout = effective_timeout
        
        try:
            return await asyncio.wait_for(
                self._scrape_internal(request),
