...
class BatchScrapeJob:
    ...
    async def execute_actions_hook(self, page, context, **kwargs):
        ...
        if not target_req:
            for k, r in self.target_requests.items():
                if k.rstrip("/") == url_key.rstrip("/") or (
                    k.split("?")[0] == url_key.split("?")[0] and k in url_key
                ):
                    target_req = r
                    break
    ...
    async def run(self) -> BatchScrapeResponse:
        ...
        response = BatchScrapeResponse(
            success=True, id=batch_id, data=[], invalid_urls=[]
        )

        valid_urls = []
        source_urls = (
            [r.url for r in self.request.requests]
            if self.request.requests
            else (self.request.urls or [])
        )
            ...
            async with AsyncWebCrawler(client=client, config=browser_config) as crawler:
                    ...
                    if not found_res:
                        found_res = next(
                            (
                                r
                                for r in results
                                if r.url.rstrip("/") == u.rstrip("/") or u in r.url
                            ),
                            None,
                        )
                    if found_res:
                        ...
                        dummy = CrawlResult(
                            ...
                            html="",
                            success=False,
                            error_message="Scrape failed or timed out without result.",
                        )
                ...
                for res in ordered_results:
                    ...
                    if not res.success:
                        ...
                        if (
                            "html" in self.request.formats
                            or "rawHtml" in self.request.formats
                        ):
                        ...
                        if "images" in self.request.formats:
                            s_resp.images = (
                                res.media.get("images", []) if res.media else []
                            )
                        ...
                        if match_url in self.action_results:
                            ...
                            for u_key, acts in self.action_results.items():
                                if u_key == getattr(
                                    res, "redirected_url", None
                                ) or u_key.rstrip("/") == res.url.rstrip("/"):
                                    acts_to_use = acts
                                    break
