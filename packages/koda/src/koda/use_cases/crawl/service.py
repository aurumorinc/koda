...
class CrawlJob:
    ...
    async def run(self) -> CrawlResponse:
        ...
        if self.request.sitemap in ("only", "include"):
            ...
            try:
                sitemap_results = await seeder.urls(
                    self.base_url, SeedingConfig(source="sitemap", extract_head=False)
                )
        ...
        browser_kwargs = {
            "headless": True,
            "proxy": self.request.scrapeOptions.proxy
            if self.request.scrapeOptions.proxy != "auto"
            else None,
        ...
        run_kwargs = {
            ...
            if self.request.scrapeOptions.storeInCache
            else CacheMode.BYPASS,
            "wait_for": f"delay:{self.request.scrapeOptions.waitFor}"
            if self.request.scrapeOptions.waitFor > 0
            else None,
        ...
        async with KodaClient() as client:
            async with AsyncWebCrawler(client=client, config=browser_config) as crawler:
                ...
                if self.request.sitemap == "only":
                    ...
                    for i in range(
                        0, min(len(urls_to_crawl), self.request.limit), chunk_size
                    ):
