...
@pytest.fixture
def mock_crawl4ai():
    with patch("koda.use_cases.batch_scrape.service.AsyncWebCrawler", autospec=True) as mock:
        yield mock
...
async def test_batch_scrape_success(mock_crawl4ai, mock_koda_client):
    ...
    req = BatchScrapeRequest(urls=["https://example.com/1", "https://example.com/2"], formats=["markdown", "html"])
